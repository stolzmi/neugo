...
func GenerateGo(modelJSON []byte, opts Options) ([]byte, error) {
	...
	varIndex := 0
	currentShape := []int{} // Will be set from first linear layer
	...
	for _, mod := range root.Modules {
		...
		case "linear":
			...
			// Set current shape for first linear layer
			if len(currentShape) == 0 {
				currentShape = []int{1, cfg.InFeatures} // batch=1, features
			}

			// Emit linear call
			fmt.Fprintf(&body, "\tcur = nnLinear(cur, %s, %s, %d, %d)\n", wVar, bVar, cfg.InFeatures, cfg.OutFeatures)
			currentShape = []int{1, cfg.OutFeatures}
			varIndex++
